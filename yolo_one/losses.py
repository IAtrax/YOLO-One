...
class YoloOneLoss(nn.Module):
    ...
    def _compute_anchor_free_box_loss(
        ...
        scale = torch.tensor([grid_w - 1, grid_h - 1], device=pred_boxes.device)
        pred_xy = torch.sigmoid(pred_boxes[:, :2])
        pred_wh = torch.sigmoid(pred_boxes[:, 2:4]) * scale
        pred_wh = pred_wh.clamp(max=scale)
        ...
        if self.iou_type == "ciou":
            ...
            pred_boxes_abs = torch.cat([pred_xy, pred_wh], dim=-1)
    ...
    def _compute_aspect_loss(
        self, pred_aspects: torch.Tensor, target_aspects: torch.Tensor
    ) -> torch.Tensor:
        """Loss for aspect ratio predictions"""
        return F.mse_loss(pred_aspects, target_aspects)

    def _compute_shape_confidence_loss(
        self, pred_shape_conf: torch.Tensor, target_shape_conf: torch.Tensor
    ) -> torch.Tensor:
        ...
        if self.focal_gamma > 0:
            ...
            pt = target_shape_conf * pred_prob + (1 - target_shape_conf) * (
                1 - pred_prob
            )
    ...
    def _compute_objectness_loss(
        self, pred_conf: torch.Tensor, target_conf: torch.Tensor, obj_mask: torch.Tensor
    ) -> torch.Tensor:
        ...
        if self.label_smoothing > 0:
            target_conf = (
                target_conf * (1 - self.label_smoothing) + 0.5 * self.label_smoothing
            )
        ...
        neg_mask = (~obj_mask).bool()

        pos_loss = (
            bce_loss[pos_mask].mean()
            if pos_mask.sum() > 0
            else torch.tensor(0.0, device=pred_conf.device)
        )
        neg_loss = (
            bce_loss[neg_mask].mean()
            if neg_mask.sum() > 0
            else torch.tensor(0.0, device=pred_conf.device)
        )
    ...
    def _ciou_loss(
        ...
        assert isinstance(
            pred_boxes, torch.Tensor
        ), f"pred_boxes must be a torch.Tensor type, got {type(pred_boxes)}"
        assert isinstance(
            target_boxes, torch.Tensor
        ...
        # Intersection
        inter_x1, inter_y1 = torch.max(pred_x1, target_x1), torch.max(
            pred_y1, target_y1
        )
        inter_x2, inter_y2 = torch.min(pred_x2, target_x2), torch.min(
            pred_y2, target_y2
        )
        ...
        # Union
        pred_area = torch.clamp(pred_x2 - pred_x1, min=0) * torch.clamp(
            pred_y2 - pred_y1, min=0
        )
        ...
        # Enclosing box
        enclosed_x1, enclosed_y1 = torch.min(pred_x1, target_x1), torch.min(
            pred_y1, target_y1
        )
        enclosed_x2, enclosed_y2 = torch.max(pred_x2, target_x2), torch.max(
            pred_y2, target_y2
        )
    ...
    def _meiou_loss(
        ...
        # Angle cost
        ch = torch.max(pred_center_y, target_center_y) - torch.min(
            pred_center_y, target_center_y
        )
        sigma = torch.sqrt(torch.abs(dist_center_2))
        delta_angle_loss = torch.where(
            sigma > 1e-6,
